...
def _signature_groups(owners, min_size):
    return _analysis.signature_groups(owners, min_size, min_shared=None)
...
def card_colors(name, scry):
    return _colors.card_colors(name, scry, include_flashback=True)


def colors_str(cards, scry):
    return _colors.colors_str(cards, scry, include_flashback=True)
...
def null_model(drafts, cube, decks, iters=2000, seed=0):
    return _analysis.null_model(drafts, cube, decks, iters, seed,
                                min_size=MIN_PACKAGE_SIZE, min_shared=None)
