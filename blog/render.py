"""Shared essay rendering and presentation for blog posts hosted on etude.gg.
...
.card { width: 160px; }
.card img { display: block; width: 100%; background: #eee; }
.card .cap { font: 12px/1.5 -apple-system, 'Segoe UI', Helvetica, sans-serif;
    color: #555; text-align: center; margin-top: 4px;
    font-variant-numeric: tabular-nums; }
.wr { font-weight: 700; font-variant-numeric: tabular-nums; }
.hi { color: #1a7a3a; } .lo { color: #b23b34; }
.big, .grouphdr { font: 15px/1.5 -apple-system, 'Segoe UI', Helvetica, sans-serif;
    color: #555; }
.grouphdr { margin: 22px 0 6px; }
th.n, td.n { text-align: right; font-variant-numeric: tabular-nums; }
a:focus-visible, button:focus-visible { outline: 2px solid #526d85;
    outline-offset: 4px; }
@media (max-width: 600px) {
    body { font-size: 18px; margin-top: 28px; padding: 0 16px; }
    .cards > img, .cards > a, .cards > .card { max-width: calc(50% - 4px); }
    .cards > a img, .card img { max-width: 100%; }
    .explorer { display: block; width: 100%; margin-left: 0; }
    .sidebar { display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 12px; }
    .sidebar button { width: auto; }
    .sidebar .sbh { width: 100%; }
    .vpanes { height: auto; max-height: 620px; }
    .seats { grid-template-columns: 1fr; }
    .vchart { overflow-x: auto; overflow-y: hidden; padding-bottom: 12px; }
    .vcol { flex-shrink: 0; }
    table { display: block; overflow-x: auto; }
    #hovercard, #deckhover { max-width: calc(100vw - 16px); }
}
...
function showPreview(e) {
  ...
  if (d && d.dataset.deck && typeof deckImgs !== 'undefined' && deckImgs[d.dataset.deck]) {
...
document.addEventListener('keydown', e => {
  if (e.key === 'Escape') { hc.style.display = 'none'; dh.style.display = 'none'; }
});
...
function activateTab(button) {
  ...
  document.querySelectorAll('[data-pane]').forEach(pane => {
    if (pane.dataset.pane === group) pane.hidden = pane.id !== button.dataset.show;
  });
  document.querySelectorAll('button[data-show]').forEach(other => {
    if (other.dataset.group !== group) return;
...
document.addEventListener('keydown', event => {
  const button = event.target.closest('button[role="tab"][data-show]');
  if (!button || !['ArrowLeft', 'ArrowRight', 'Home', 'End'].includes(event.key)) return;
  const buttons = [...button.closest('[role="tablist"]').querySelectorAll('[role="tab"]')];
  let i = buttons.indexOf(button);
  i = event.key === 'Home' ? 0 : event.key === 'End' ? buttons.length - 1
    : (i + (event.key === 'ArrowRight' ? 1 : -1) + buttons.length) % buttons.length;
  event.preventDefault();
  activateTab(buttons[i]);
  buttons[i].focus();
});
</script>"""


def card_gallery(cards, images, *, captions=None, small=False):
    """Linked card art with optional HTML captions; preserve caller order."""
    cls = " small" if small else ""
    out = [f"<div class='cards{cls}'>"]
    for card in cards:
        image = images.get(card)
        if not image:
            raise ValueError(f"Missing image for {card}")
        escaped = html.escape(card, quote=True)
        url = html.escape(image, quote=True)
        target = "https://scryfall.com/search?q=!%22" + quote(card) + "%22"
        art = (f"<a href='{target}' data-img='{url}' target='_blank' rel='noopener'>"
               f"<img src='{url}' alt='{escaped}' title='{escaped}' loading='lazy'></a>")
        if captions is not None:
            art = f"<div class='card'>{art}<div class='cap'>{captions[card]}</div></div>"
        out.append(art)
    out.append("</div>")
    return "".join(out)


def tabbed_panels(group, panels, *, label):
    """Accessible tabs: panels are (plain-text label, generated HTML) pairs."""
    group = html.escape(group, quote=True)
    out = [f"<div class='tabs' role='tablist' aria-label='{html.escape(label, quote=True)}'>"]
    panes = []
    for i, (name, body) in enumerate(panels):
        tab, pane = f"{group}-tab-{i}", f"{group}-pane-{i}"
        selected, hidden = ('true', '') if i == 0 else ('false', ' hidden')
        on = " class='on'" if i == 0 else ""
        out.append(f"<button type='button' id='{tab}' role='tab'{on} data-group='{group}' "
                   f"data-show='{pane}' aria-controls='{pane}' aria-selected='{selected}' "
                   f"tabindex='{0 if i == 0 else -1}'>{html.escape(name)}</button>")
        panes.append(f"<div id='{pane}' role='tabpanel' data-pane='{group}' "
                     f"aria-labelledby='{tab}' tabindex='0'{hidden}>{body}</div>")
    return "".join(out) + "</div>" + "".join(panes)
