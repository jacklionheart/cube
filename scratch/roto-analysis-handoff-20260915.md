...
- **MUST always be updated in place** (`upload_sheet.py --update <id>`,
  ...
  file for it.
- Tab order: Pick Summary, Color Analysis, Lanes (all 3 drafts),
  Teams (2 of 3), Card List, Draft 1–3, Records,
...
- Relaxed package definitions (n−k1 of the cards in one deck, in n−k2
  ...
  meaningful). Two analyses ship in the sheet: Lanes (all 3 drafts) (strict >=3 cores merged with their ±1 flex: Theme/Colors/Core/Flex/All + owners + record) and Teams (2 of 3) (pair cores, min size 5 — Jack's vocabulary, do not rename: a Lane = strict core + flex orbit (a recurring deck skeleton); a Team = the unit of cards two decks agreed on; packages.py --straddles lists decks pairing with >=2 decks of the same other draft — the merge/split seams).
